...
class Player extends EventEmitter {
    ...
    constructor() {
        ...
        this.parseThreads = Math.max(1, os.availableParallelism() - 1) // leave a thread for main
            ...
            try {
                ...
                for (let i = 0; i < this.#numTracks; ++i) {
                    if (currentOffset >= buf.length) {
                        throw new Error(
                            `Reached EOF while looking for track ${i}. Tracks reported in header: ${this.#numTracks}.`
                        )
                    }
                    ...
                    if (trackMagic !== 0x4d54726b) {
                        // MTrk
                        throw new Error(
                            `Invalid track ${i} magic! Expected 4d54726b, got ${trackMagic.toString(16).padStart(8, '0')}.`
                        )
                    }
                ...
                const tracksWithSize = this.#trackOffsets.map((offset, index) => ({
                    index,
                ...
                for (let i = 0; i < numWorkers; ++i) {
                    ...
                    worker.on('online', () => (worker.active = true))
                    ...
                    worker.on('message', (t) => {
                        ...
                        this.#tempoEvents.push(t.tempoEvents)
                        ++this.#tracksParsed
                    })
    ...
    #playLoop() {
        ...
        if (
            this.#tracks.every((track, i) => this.#trackEventPointers[i] >= track.eventCount) ||
